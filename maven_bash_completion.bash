...
__pom_hierarchy()
{
    ...
    POM_HIERARCHY+=("$pom")
    while true; do
		# im <parent> Block wird nach <relativePath> gesucht und ausgegeben
		# wird der nicht gefunden, wird "../pom.xml" ausgegeben
		# gibt es keinen <parent> Block, wird nichts ausgegeben
		local parent_pom_relative=`sed -n "/<parent>/{:l;/<\/parent>/be;N;bl;:e;s/.*<relativePath>\(.*\)<\/relativePath>.*/\1/p;tq;s/.*/..\/pom.xml/p;:q;q}" "$pom"`

    	parent_pom=${pom%/*}/$parent_pom_relative
    	## if parent_pom exists continue else break
...
#	common_plugins=`compgen -v | grep "^plugin_goals_.*" | sed 's/plugin_goals_//g' | tr '_' '-' | tr '\n' '|'`
	common_plugins=`compgen -v | sed -n 's/^plugin_goals_//g;\${g;s/^\n//;s/\$/\n/;y/_\n/-\|/;p;q};T;H'`
}
...
#    local profile_settings=`[ -e ~/.m2/settings.xml ] && grep -e "<profile>" -A 1 ~/.m2/settings.xml | grep -e "<id>.*</id>" | sed 's/.*<id>//' | sed 's/<\/id>.*//g' | tr '\n' '|' `
    ...
    if [[ ${cur} == -D* ]] ; then
      COMPREPLY=( $(compgen -S ' ' -W "${options}" -- ${cur}) )
		...
		do
		#echo $item	
		#        local profile_pom=`[ -e $item ] && sed -n "/<profile>/{N;s/.*<id>\(.*\)<\/id>.*/\1/p}" $item | tr '\n' '|' `
		# alle Profile IDs in den sed-holdspace; fuehrende newline entfernen und am Ende eine setzen; newlines durch pipe ersetzen
			local profile_pom=`sed -n '/<profile>/{N;s/.*<id>\(.*\)<\/id>.*/\1/;H};\${g;s/^\n//;s/\$/\n/;s/\n/|/gp}' $item `
...
#complete -o default -F _mvn_time -o nospace mvn
